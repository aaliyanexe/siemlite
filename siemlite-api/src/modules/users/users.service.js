...
async function getUserById(userId) {
  ...
  if (!user) {
    throw new AppError('USER_NOT_FOUND', `User with id ${userId} does not exist`, 404);
  }
...
async function updateUser(userId, data, actingUserId) {
  ...
  if (data.role === 'Analyst' && user.role === 'Admin' && userId === actingUserId) {
    const adminCount = await usersRepo.countActiveAdmins();
    if (adminCount <= 1) {
      throw new AppError(
        'LAST_ADMIN',
        'Cannot demote yourself — you are the only active Admin',
        403
      );
    }
  }
  ...
  if (data.role === 'Analyst' && user.role === 'Admin') {
    ...
    if (adminCount <= 1) {
      throw new AppError(
        'LAST_ADMIN',
        'Cannot demote the only active Admin in the system',
        403
      );
    }
  }
...
async function deactivateUser(userId, actingUserId) {
  ...
  if (!user.is_active) {
    throw new AppError('USER_INACTIVE', 'User is already deactivated', 400);
  }

  if (user.role === 'Admin') {
    const adminCount = await usersRepo.countActiveAdmins();
    if (adminCount <= 1) {
      throw new AppError('LAST_ADMIN', 'Cannot deactivate the only active Admin', 403);
    }
  }

  if (userId === actingUserId) {
    throw new AppError('CANNOT_DEACTIVATE_SELF', 'Cannot deactivate your own account', 403);
  }
