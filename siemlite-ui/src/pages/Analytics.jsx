...
import {
  ...
  RefreshCw, ChevronDown, ExternalLink, Activity,
...
const ChartTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{
      ...
      <p style={{ color: 'var(--text-tertiary)', marginBottom: 6, fontSize: 11, letterSpacing: '0.04em' }}>{label}</p>
      {payload.map((p, i) => (
      ...
      <div style={{
        ...
        <div style={{
          ...
          <button onClick={onClose} style={{
            ...
            color: T.textTertiary, fontSize: 18, lineHeight: 1,
            padding: '0 4px',
          }}>×</button>
        </div>
...
export default function Analytics() {
  ...
  const [drill, setDrill]         = useState(null); // { title, incidents }
  const refreshTimer = useRef(null);

  const fetchAll = useCallback(async () => {
    // Don't flash to empty — only show loading on first fetch
    ...
    const [
      threatRes, slaRes, trendRes, assetRes, workloadRes, breachRes,
    ...
    // Batch all state updates in one synchronous block so React
    // flushes them together — prevents intermediate blank renders
  ...
  const overallSLA = (() => {
    ...
    const withinSla = sla.by_severity.reduce((s, r) => s + (r.within_sla || 0), 0);
    return total > 0 ? Math.round((withinSla / total) * 100) : 0;
  })();
  const topThreat  = threats[0]?.threat_type || '—';
  const totalBreached = breached.length;

  // Avg workload for flagging
  const avgIncidentCount = workload.length
    ? workload.reduce((s, w) => s + (Number(w.incidents_handled ?? w.open_incidents) || 0), 0) / workload.length
    : 0;
  ...
  return (
    <PageWrapper>
      ...
      <div style={{
        ...
        justifyContent: 'space-between',paddingTop: '12px', paddingBottom: '16px',
      }}>
        <div>
          <h1 style={{
          ...
          <div style={{
            ...
            {RANGE_OPTIONS.map(opt => (
              <button key={opt.value} onClick={() => setRange(opt.value)} style={{
                ...
                fontFamily: 'PP Neue Montreal, system-ui, sans-serif',
                transition: 'all 120ms ease',
                borderLeft: range === opt.value ? `1px solid ${T.borderDefault}` : '1px solid transparent',
              }}>{opt.label}</button>
            ))}
          </div>
      ...
      >
        {loading ? (
          ...
          <div style={{ height: 240, display: 'flex', alignItems: 'center', justifyContent: 'center', color: T.textTertiary, fontSize: 13 }}>No trend data for this period</div>
        ) : (
          <>
            {/* Legend */}
      ...
      {/* ── Row 2: Threat Frequency + SLA Compliance ── */}
        ...
        <Card title="Threat Frequency" subtitle="click a bar to drill down">
          {loading ? (
            ...
            <ResponsiveContainer width="100%" height={320}>
              ...
              >
                <XAxis type="number" tick={{ fill: '#5c4a32', fontSize: 12 }} tickLine={false} axisLine={false} />
                ...
                <Bar dataKey="total" name="Incidents" radius={[0, 4, 4, 0]} cursor="pointer" barSize={18}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
        ...
        {/* Asset Exposure */}
        <Card title="Asset Exposure" subtitle="severity-weighted incident score · click to drill down">
          {loading ? (
          ...
          {loading ? (
            ...
            <div style={{ overflowY: 'auto', maxHeight: 260 }}>
              ...
              <div style={{
                ...
                {['Analyst','Open','Actions','Avg TTR'].map(h => (
                  <span key={h} style={{
                    fontSize: 9, fontWeight: 500, letterSpacing: '0.08em',
                    textTransform: 'uppercase', color: T.textTertiary,
                    fontFamily: 'PP Neue Montreal, system-ui, sans-serif',
              ...
              {workload.map((w, i) => {
                const aboveAvg = Number(w.incidents_handled ?? w.open_incidents) > avgIncidentCount * 1.3;
                return (
                  <div key={w.analyst_id || i} style={{
                    display: 'grid', gridTemplateColumns: '1fr 60px 60px 70px',
                    padding: '9px 0',
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <div style={{
                        ...
                        {(w.name || w.analyst_name || 'U').slice(0,2).toUpperCase()}
                      </div>
                      <span style={{
                        fontSize: 12, fontWeight: 500, color: T.textPrimary,
                        fontFamily: 'PP Neue Montreal, system-ui, sans-serif',
                      }}>{w.name || w.analyst_name || '—'}</span>
                      {aboveAvg && (
                        <span style={{
      ...
      >
        {loading ? (
          <Skeleton width="60%" height={13} radius={4} style={{ marginBottom: 8 }} />
        ) : breached.length === 0 ? (
          <div style={{
            ...
            <Shield size={20} style={{ color: '#22C55E', marginBottom: 8, display: 'block', margin: '0 auto 8px' }} />
          ...
          <>
            ...
            <div style={{
              display: 'grid',
              gridTemplateColumns: '1.8fr 100px 130px 1fr 100px',
              padding: '0 0 12px',
              borderBottom: `1px solid ${T.borderSubtle}`,
              marginBottom: 4,
            }}>
              ...
              return (
                <div key={inc.incident_id} style={{
                  ...
                  onMouseEnter={e => {
                    ...
                    e.currentTarget.style.borderLeftColor = '#FF3B3B';
                  }}
                  onMouseLeave={e => {
      ...
      {/* ── Spin keyframe ── */}
      <style>{`@keyframes spin { to { transform: rotate(360deg); } } @keyframes skelPulse { 0%,100% { opacity:1; } 50% { opacity:0.4; } }`}</style>
    </PageWrapper>
  );
}